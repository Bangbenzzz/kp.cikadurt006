...
const COLORS_AGE = ['#8884d8', '#83a6ed', '#8dd1e1', '#82ca9d', '#a4de6c'];
...
export default function DashboardHome() {
  ...
  const stats = useMemo(() => {
      ...
      const catCounts = { "Balita":0, "Anak":0, "Remaja":0, "Dewasa":0, "Lansia":0 };
      active.forEach(w => {
      ...
      const latest = [...active].slice(-5).reverse();
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem', width: '100%', maxWidth: '100%', overflowX: 'hidden' }}>
        ...
        {/* --- HEADER (TANGGAL LENGKAP & KECIL) --- */}
        <div style={{ 
            display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', 
            gap: '0.5rem', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '0.5rem' 
        }}>
            <div>
                <h1 style={{ margin: 0, fontSize: '1.1rem', fontWeight: '700', color: '#e0e0e0' }}>Dashboard RT</h1>
                <p style={{ margin: 0, fontSize: '0.65rem', color: '#888' }}>Monitoring Realtime</p>
            </div>
            <div style={{ textAlign: 'right', display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                {/* TANGGAL LENGKAP DISINI */}
                <div style={{ fontSize: '0.7rem', color: '#aaa' }}>
                    {time.toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}
                </div>
                <div style={{ fontSize: '0.9rem', fontWeight: '600', color: '#00eaff', fontFamily: 'monospace' }}>
                    {time.toLocaleTimeString('id-ID', {hour: '2-digit', minute:'2-digit'})} WIB
                </div>
            </div>
        </div>

        {/* --- KARTU STATISTIK (EXTRA COMPACT) --- */}
        <div style={{ 
            display: 'grid', 
            gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', // Lebih kecil min-width nya agar muat 4 di HP lebar
            gap: '0.6rem' 
        }}>
            <CardStat icon={<FaUsers />} label="Total" value={stats.total} sub="Jiwa" color="#00eaff" bg="rgba(0, 234, 255, 0.1)"/>
        ...
        {/* --- AREA GRAFIK (COMPACT & FIXED HEIGHT) --- */}
        <div style={{ 
            display: 'grid', 
            ...
            <div style={containerStyle}>
                <h3 style={headerStyle}>
                    <FaChild style={{color: '#8884d8'}}/> Usia
                </h3>
                {/* Height FIX 180px agar tidak error */}
                <div style={{ width: '100%', height: '180px', minWidth: 0 }}>
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={stats.ageData} margin={{top:5, right:5, left:-25, bottom:0}}>
                            ...
                            />
                            <Bar dataKey="jumlah" fill="#8884d8" radius={[3, 3, 0, 0]} barSize={25}>
            ...
            <div style={containerStyle}>
                <h3 style={headerStyle}>
                    <FaUserTie style={{color: '#00C49F'}}/> Gender
                </h3>
                {/* Height FIX 180px agar tidak error */}
                <div style={{ width: '100%', height: '180px', minWidth: 0 }}>
                    <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                            <Pie
                                ...
                                cx="50%" cy="50%"
                                innerRadius={40} // Sangat ramping
                                outerRadius={60} // Kecil
                                paddingAngle={5}
                                dataKey="value"
                                stroke="none"
                            >
        ...
        {/* --- DAFTAR WARGA TERBARU (COMPACT) --- */}
        <div style={containerStyle}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.6rem' }}>
                <h3 style={{ margin: 0, color: '#e0e0e0', fontSize: '0.8rem' }}>Warga Baru</h3>
            </div>
                ...
                {stats.latest.length > 0 ? stats.latest.map((w, i) => (
                    <div key={i} style={{ 
                        ...
                        <div style={{ 
                            ...
                            background: w.jenis_kelamin === 'L' || w.jenis_kelamin === 'Laki-laki' ? 'linear-gradient(135deg, #0052cc, #007aff)' : 'linear-gradient(135deg, #d4145a, #fbb03b)',
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontWeight: '600', color: '#fff', fontSize: '0.75rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{w.nama}</div>
                            <div style={{ fontSize: '0.65rem', color: '#777' }}>
                                {w.status} • {getAge(w.tgl_lahir)} Thn
                            </div>
                        </div>
